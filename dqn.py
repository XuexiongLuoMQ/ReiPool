...
class ThreshEstimator(Estimator):
    def __init__(self, action_num, lr, state_shape, mlp_layers, device):
        super(ThreshEstimator, self).__init__(action_num, lr, state_shape, mlp_layers, device)
    ...
    def predict_nograd(self, states):
        with torch.no_grad():
            ...
            if len(states.shape) < 2:
                states = states.unsqueeze(0)
            q_values = torch.sigmoid(self.qnet(states)).to('cpu').numpy()
        return q_values
