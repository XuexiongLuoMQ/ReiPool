import numpy as np
from dqn import Memory, ThreshEstimator
...
class ThresholdAgent(object):
    def __init__(self,
        ...
        self.device = device
        self.train_t = 0
        self.q_estimator = ThreshEstimator(action_num=action_num, lr=lr, state_shape=self.state_shape, mlp_layers=mlp_layers, device=device)
        self.target_estimator = ThreshEstimator(action_num=action_num, lr=lr, state_shape=self.state_shape, mlp_layers=mlp_layers, device=self.device)
